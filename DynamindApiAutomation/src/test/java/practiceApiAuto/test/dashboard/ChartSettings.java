...
public class ChartSettings extends Base {
    Response response;
    int chartsLength;

    @Test
    public void testGetChartsForSettings(){
        response= EndPoints.chartSettings();
        if(response.jsonPath().getInt("statusCode")==200 && response.jsonPath().getString("status").equals("OK")){
            System.out.println("All Charts Found Successfully for settings on dashboard....");
        }else {
    ...
    @Test(dependsOnMethods = "testGetChartsForSettings")
